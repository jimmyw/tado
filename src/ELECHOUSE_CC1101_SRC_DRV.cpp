...
void ELECHOUSE_CC1101::SpiWriteReg(byte addr, byte value) {
  ...
  Serial.printf("%02X", addr & 0xFF); // Ensures only two digits are printed
  Serial.print(" Value: 0x");
  Serial.printf("%02X", value & 0xFF);
  Serial.println();
...
void ELECHOUSE_CC1101::setModulation(byte m) {
  Serial.printf("Set modulation %d\r\n", m);
  if (m > 4) {
    m = 4;
  }
  modulation = m;
...
void ELECHOUSE_CC1101::setSyncWord(byte sh, byte sl) {
  Serial.printf("Set sync word 0x%x 0x%x\r\n", sh, sl);
...
void ELECHOUSE_CC1101::setAppendStatus(bool v) {
  Serial.printf("setAppendStatus %d\r\n", v);
  Split_PKTCTRL1();
  pc1APP_ST = 0;
  if (v == 1) {
    pc1APP_ST = 4;
...
void ELECHOUSE_CC1101::setLengthConfig(byte v) {
  Serial.printf("setLengthConfig %d\r\n", v);
  Split_PKTCTRL0();
  pc0LenConf = 0;
  if (v > 3) {
    v = 3;
...
void ELECHOUSE_CC1101::setPacketLength(byte v) {
  Serial.printf("setPacketLength %d\r\n", v);
...
void ELECHOUSE_CC1101::setDcFilterOff(bool v) {
  Serial.printf("setDcFilterOff %d\r\n", v);
  Split_MDMCFG2();
  m2DCOFF = 0;
  if (v == 1) {
    m2DCOFF = 128;
...
void ELECHOUSE_CC1101::setSyncMode(byte v) {
  Serial.printf("Set sync mode 0x%x\r\n", v);
  Split_MDMCFG2();
  m2SYNCM = 0;
  if (v > 7) {
    v = 7;
...
void ELECHOUSE_CC1101::setChsp(float f) {
  Serial.printf("Set channel spacing %f\r\n", f);
  Split_MDMCFG1();
  byte MDMCFG0 = 0;
  m1CHSP = 0;
  if (f > 405.456543) {
...
void ELECHOUSE_CC1101::setRxBW(float f) {
  ...
  Serial.printf("E: %x  M: %x\r\n", s1, s2);
  s1 *= 64;
  s2 *= 16;
  m4RxBw = s1 + s2;
  Serial.printf("m4RxBw: %x\r\n", m4RxBw);
...
void ELECHOUSE_CC1101::setDRate(float d) {
  ...
  Serial.printf("m4RxBw: %x\r\n", m4RxBw);
  Serial.printf("m4DaRa: %x\r\n", m4DaRa);
  SpiWriteReg(CC1101_MDMCFG4, m4RxBw + m4DaRa);
